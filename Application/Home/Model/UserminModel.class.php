...
class UserminModel extends BaseModel{
    ...
    public function addUser($data){
        $query = "insert into usermin (username,password,email,administrator) values ('{$data['user']}','{$data['password']}','{$data['email']}',{$data['administrator']})";
    ...
    public function queryadministrator($name){//判断是不是管理员是管理员返回真
        $sql = "select administrator from usermin where username = '{$name}'";
        $result  = $this->query($sql);
